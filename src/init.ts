 ...
 * env, Postgres, Redis, Mongo. Express-free on purpose — the worker must not
 * pull in the HTTP stack. Returns the loaded config + a teardown that
...
export async function initCoreServices(
  ...
  logger.info("Initializing core services (env, Postgres, Redis, Mongo)");
